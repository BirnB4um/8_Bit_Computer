...
#file = input("code file to compile: ")
file = "code.txt"
...
for line in ls:
    ...
    if not (line.replace("\t", "").replace(" ", "") == ""):#if no empty line
        ...
        if line.split(" ")[0] == "var":#variable definition
...
def write_board(code_data):

    block_one_start_x = 122
    block_one_start_y = 178
    block_two_start_x = 321
    block_two_start_y = 178
    offset_x = 4
    offset_y = 9

    empty_computer = []
    with open("empty_computer.board", 'rb') as file:
        empty_computer = file.read()

    board_width = int.from_bytes(empty_computer[1:5], byteorder='little')

    empty_computer = list(empty_computer)

    #first block
    for i in range(0, 128, 1):
        for bit in range(0, 35, 1):
            if code_data[i][bit] == 1:
                empty_computer[9 + ((block_one_start_y + i * offset_y) * board_width + block_one_start_x + bit * offset_x) * 4] = 3
                empty_computer[9 + ((block_one_start_y + i * offset_y) * board_width + block_one_start_x + bit * offset_x) * 4 + 1] = 2
                empty_computer[9 + ((block_one_start_y + i * offset_y) * board_width + block_one_start_x + bit * offset_x) * 4 + 2] = 0
                empty_computer[9 + ((block_one_start_y + i * offset_y) * board_width + block_one_start_x + bit * offset_x) * 4 + 3] = 0
            else:
                empty_computer[9 + ((block_one_start_y + i * offset_y) * board_width + block_one_start_x + bit * offset_x) * 4] = 0
                empty_computer[9 + ((block_one_start_y + i * offset_y) * board_width + block_one_start_x + bit * offset_x) * 4 + 1] = 0
                empty_computer[9 + ((block_one_start_y + i * offset_y) * board_width + block_one_start_x + bit * offset_x) * 4 + 2] = 0
                empty_computer[9 + ((block_one_start_y + i * offset_y) * board_width + block_one_start_x + bit * offset_x) * 4 + 3] = 0
    ...
    for n in range(128, 256, 1):
        i = n - 128
        for bit in range(0, 35, 1):
            if code_data[n][bit] == 1:
                empty_computer[9 + ((block_two_start_y + i * offset_y) * board_width + block_two_start_x + bit * offset_x) * 4] = 3
                empty_computer[9 + ((block_two_start_y + i * offset_y) * board_width + block_two_start_x + bit * offset_x) * 4 + 1] = 2
                empty_computer[9 + ((block_two_start_y + i * offset_y) * board_width + block_two_start_x + bit * offset_x) * 4 + 2] = 0
                empty_computer[9 + ((block_two_start_y + i * offset_y) * board_width + block_two_start_x + bit * offset_x) * 4 + 3] = 0
            else:
                empty_computer[9 + ((block_two_start_y + i * offset_y) * board_width + block_two_start_x + bit * offset_x) * 4] = 0
                empty_computer[9 + ((block_two_start_y + i * offset_y) * board_width + block_two_start_x + bit * offset_x) * 4 + 1] = 0
                empty_computer[9 + ((block_two_start_y + i * offset_y) * board_width + block_two_start_x + bit * offset_x) * 4 + 2] = 0
                empty_computer[9 + ((block_two_start_y + i * offset_y) * board_width + block_two_start_x + bit * offset_x) * 4 + 3] = 0

    empty_computer = bytes(empty_computer)
    with open("output.board", 'wb') as file:
        file.write(empty_computer)


print("writing code into ROM...")
write_board(compiled_program)
input("done")